 ...
 * - base: 2..36
 * - returns pointer to buf on success, NULL on error (invalid base or buffer too small)
 *
 * Notes:
...
char* itoa_bare(char *buf, size_t bufsize, long value, int base) {
    ...
    if (value == 0) {
        if (bufsize < 2) return NULL; // need at least "0\0"
        buf[0] = '0';
        buf[1] = '\0';
        return buf;
    }
    ...
    // worst-case digits count for base 2 of 64-bit is 64, add 1 for sign and 1 for NUL
    // we will write directly into user buffer from the end to avoid extra alloc,
    // but simpler to use a small local temp array sized to the max needed.
    ...
    while (uvalue && ti < sizeof(tmp)) {
        ...
        else tmp[ti++] = (char)('a' + (rem - 10)); // lowercase letters
...
char* utoa_bare(char *buf, size_t bufsize, unsigned long value, int base) {
    ...
    for (size_t j = 0; j < ti; j++) buf[j] = tmp[ti - 1 - j];
    buf[ti] = '\0';
    return buf;
}
